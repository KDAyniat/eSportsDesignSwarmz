...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	if($id == "" || strlen($u) < 3 || strlen($e) < 5 || $p == ""){
		...
		mail('info@esportsdesignswarm.com','Activation Error','Activation String Length Issues',$headers);
    	exit();
	}
	...
	if($numrows == 0){
		...
		header("location: message.php?msg=Your credentials are not matching anything in our system");
    	          ...
    	          $headers .= "MIME-Version: 1.0\n";
                        $headers .= "Content-type: text/html; charset=iso-8859-1\n";
        		mail('info@esportsdesignswarm.com','Credentials Error! Possible Hack attempt','Credentials Error! Possible Hack Attempt',$headers);
    	exit();
	}
    ...
    if($numrows == 0){
    	...
    	$headers = "From: $from\n";
                        $headers .= "MIME-Version: 1.0\n";
                        $headers .= "Content-type: text/html; charset=iso-8859-1\n";
        		mail('info@esportsdesignswarm.com','Activation Error','Activation failed to save to database for',$headers);
    	exit();
    } else if($numrows == 1) {
	...
	header("location: message.php?msg=missing_GET_variables");
                    ...
                    $headers .= "MIME-Version: 1.0\n";
                    $headers .= "Content-type: text/html; charset=iso-8859-1\n";
    		        mail('info@esportsdesignswarm.com','Activation Error','Missing initial $_GET variables',$headers);
    exit();
}
?>
